...
public class ServicioGestionEmpleados {
    ...
    public AsignacionTurno asignarTurno(String identificacionEmpleado, LugarTrabajo lugarTrabajo, LocalDate fecha, Turno turno) {
        ...
        if (lugarTrabajo != null) {
            ...
            // lugarTrabajo.asignarEmpleado ya lanza AsignacionInvalidaException o CapacitacionInsuficienteException
            try {
                 ...
                 System.out.println("Asignación creada: " + empleado.getNombre() + " a " + lugarTrabajo.getNombreLugar() + " el " + fecha + " (" + turno + ")");
            } catch (CapacitacionInsuficienteException e) { // Catch specific exception first
                 // Re-lanzar la excepción específica capturada
                 throw e;
            } catch (AsignacionInvalidaException e) { // Catch parent exception
                 // Re-lanzar la excepción específica capturada
                 throw e;
            } catch (DatosInvalidosException e) { // Capturar si asignarEmpleado lanza por empleado null (aunque ya validamos)
    ...
    public boolean verificarRequisitosPersonalLugar(LugarTrabajo lugarTrabajo, LocalDate fecha, Turno turno) {
        ...
        // 2. Verificar si todos los asignados son válidos para ese lugar (tipo y capacitaciones generales)
        boolean todosSonAsignables = empleadosAsignadosEsteTurno.stream()
                                        .allMatch(lugarTrabajo::puedeAsignarEmpleado); // Usa la lógica de puedeAsignarEmpleado

        if (!todosSonAsignables) {
             System.out.println("Incumplimiento Requisitos: Algunos empleados asignados a " + lugarTrabajo.getNombreLugar() + " para " + fecha + " " + turno + " no cumplen los requisitos básicos del puesto (tipo/capacitación general).");
             // Podríamos detallar quién no cumple si fuera necesario
            return false;
        }

        // 3. Verificaciones específicas de roles requeridos para el turno
        boolean requisitosEspecificosCumplidos = true;
        String mensajeIncumplimiento = "";

        if (lugarTrabajo instanceof Tienda || lugarTrabajo instanceof Taquilla) { // Check type Taquilla
             ...
             if (!tieneCajeroCapacitado) {
                 ...
                 mensajeIncumplimiento = lugarTrabajo.getClass().getSimpleName() + " '" + lugarTrabajo.getNombreLugar() + "' no tiene un Cajero capacitado asignado.";
             }
        } else if (lugarTrabajo instanceof Cafeteria) {
             boolean tieneCajeroCapacitado = empleadosAsignadosEsteTurno.stream()
                                             .anyMatch(e -> e instanceof Cajero && e.tieneCapacitacion(Capacitacion.MANEJO_CAJA));
             boolean tieneCocinero = empleadosAsignadosEsteTurno.stream()
                                             .anyMatch(e -> e instanceof Cocinero); // Asume Cocinero tiene capacitaciones necesarias verificadas por puedeAsignarEmpleado

             if (!tieneCajeroCapacitado || !tieneCocinero) {
                 requisitosEspecificosCumplidos = false;
                 mensajeIncumplimiento = "Cafetería '" + lugarTrabajo.getNombreLugar() + "' requiere al menos un Cocinero y un Cajero capacitado.";
             }
        }
        // Atracciones: La verificación de OperarioAtraccion y sus capacitaciones específicas
        // ya está cubierta por el chequeo de puedeAsignarEmpleado.

        if (!requisitosEspecificosCumplidos) {
            System.out.println("Incumplimiento Requisitos Específicos: " + mensajeIncumplimiento + " para " + fecha + " " + turno);
            return false;
        }
